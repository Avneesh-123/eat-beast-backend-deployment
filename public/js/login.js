...
loginBtn.addEventListener("click", async function(e){
    ...
    try{
        ...
        if(email.value && password.value){
            let obj = await axios.post( "http://eat-beast-herk.herokuapp.com/login" , {email:email.value , password:password.value});
            console.log(obj);
            if(obj.data.data){
...
createAccBtn.addEventListener("click",async function(e){
    ...
    try{
        if(emailSu && passwordSu && confirmPasswordSu && Username){
            let obj = await axios.post( "http://eat-beast-herk.herokuapp.com/login" , {name:Username.value ,email:emailSu.value , password:passwordSu.value , confirmPassword:confirmPasswordSu.value});
            console.log(obj);
        }
    }catch(error){
        console.log(error);
